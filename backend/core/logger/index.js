 ...
 * Designed for AWS CloudWatch Logs compatibility.
 */
...
function formatEntry(level, module, message, meta = {}) {
  const entry = {
    ...
    module,
    message,
    requestId: meta.requestId || process.env.AWS_REQUEST_ID || undefined,
    ...meta,
  };
 ...
 * @param {string} moduleName - e.g. 'core:auth', 'functions:user:createUser'
...
function createLogger(moduleName) {
  return {
    ...
    /**
     * Log an incoming Lambda event (sanitized)
     * @param {object} event
     */
    logEvent: (event) => {
      ...
      write('debug', moduleName, 'Lambda event received', { event: sanitized });
    },
  };
}
