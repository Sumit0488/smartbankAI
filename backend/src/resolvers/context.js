 ...
 * 2. Falls back to raw base64-decode for external tokens
 */
...
function buildContext({ req }) {
  ...
  if (payload) {
    // AppSync-compatible identity shape expected by extractIdentity() in utils/auth
    identity = {
      sub:      payload.sub,
